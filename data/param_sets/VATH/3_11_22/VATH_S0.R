...
start_date_u_f <- rtruncnorm(1,a=1,b=365,mean=260,sd=20)

start_date_sd_f <- rtruncnorm(1,a=0,mean=15,sd=10)

#Orientation parameters - means should be 0
bear_err_mean_s <- rnorm(1,0,10)
bear_err_sd_s <- rtruncnorm(1,a=0,mean=40,sd=30)
bear_err_mean_f <- rnorm(1,0,10)
bear_err_sd_f <- rtruncnorm(1,a=0,mean=40,sd=30)

#Energetics info
max_energy_s <- rtruncnorm(1,a=0,mean=5000,sd=2000)
max_energy_f <- rtruncnorm(1,a=0,mean=5000,sd=2000)
recovery_rate_s <- rtruncnorm(1,a=0,b=1,mean=.1,sd=.1)
recovery_rate_f <- rtruncnorm(1,a=0,b=1,mean=.1,sd=.1)
